...
faixa_cor='33';1'
texto_cor='32'
caixa_cor='33;43'	# Depois descomente essa linha para surpresa
...
texto_coluna_inicio=$((caixa_coluna_inicio+distancia_borda_texto))
...
### Desenha a caixa ##
echo -n "\033[$caixa_linha_inicio;0H"	# Pula para a linha inicial
echo -n "\033[${caixa_cor}m"	# Liga a cor da caixa
for i in $(seq $total_linhas);do	# Para cada linha faça
	echo -n "\033[${caixa_coluna_inicio}G"	# Vai p/coluna inicial
	if [ $i -eq 1 -o $i -eq $total_linhas ]; then	# 1a ou última
...
echo -n "\033[$((caixa_linha_inicio+1));0H"   # Pula para a linha inicial
echo -n "\033[${texto_cor}m"    # Liga a cor da caixa
